...
Feature: Meet bounded real-time and scale budgets
  ...
  @MTG-180 @red @native @hardware @performance
  ...
  @MTG-181 @red @domain @performance
  ...
  @MTG-182 @red @native @performance
  Scenario: A thirty-second provider stall leaves capture healthy
    Given live transcription and dual-channel recording are active
    When the provider consumes no audio for thirty seconds
    Then the real-time callback and recorder stay within their budgets
    And transcription exposes backlog, bounded replay, or an explicit gap

  @MTG-183 @red @renderer @performance @accessibility
  ...
  @MTG-184 @red @native @performance
  ...
  @MTG-185 @red @native @performance
  ...
  @MTG-186 @red @contract @performance
