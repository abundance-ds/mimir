...
Feature: Ship the meeting engine as a supported and auditable desktop capability
  ...
  @MTG-220 @manual @desktop @release @hardware
  ...
  @MTG-221 @automated @packaging @security
  Scenario: The packaged app declares only required capture capabilities
    ...
    Then microphone and system-audio access match the implementation
    And no development-only or unrelated broad capability is present

  @MTG-222 @automated @native @contract
  ...
  @MTG-223 @automated @native @contract
  ...
  @MTG-224 @manual @packaging @security @supply-chain @release
  ...
  @MTG-225 @automated @packaging @security
  ...
  @MTG-226 @manual @release @contract
  ...
  @MTG-227 @manual @release @recovery
  Scenario: Disabling Scribe preserves user data
    Given meeting data exists after enabling the capability
    When meeting detection and background processing are disabled
    Then capture and background jobs remain disabled without deleting meeting data
    And the user can still inspect and export compatible meeting content

  @MTG-228 @manual @release @security
