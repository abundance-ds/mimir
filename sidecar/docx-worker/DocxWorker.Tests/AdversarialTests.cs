...
public class AdversarialTests : IDisposable
{
    ...
    [Fact]
    public void TableOnlyDocument_ShouldThrowNotCorrupt()
    {
        // LIMITATION: TextSearcher only searches body-level paragraphs, not table cells
        var input = CreateDocxWithBody("table-only.docx", body =>
        {
            var table = new Table(
                new TableRow(
                    new TableCell(new Paragraph(new Run(new Text("Cell text that should not be found"))))));
            body.Append(table);
        });
        ...
        {
            Assert.Throws<ArgumentException>(() =>
                session.AddCommentByText("Cell text", "Author", "Table text"));
        }
    }
