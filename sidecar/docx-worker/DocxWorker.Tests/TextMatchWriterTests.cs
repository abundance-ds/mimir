...
public class TextMatchWriterTests : IDisposable
{
    ...
    {
        // TextSearcher.FindText searches body-level paragraphs only.
        // Table cell text is not searched by FindText, so AddCommentByText
        // should throw for text that only exists in a table.
        var inputPath = Path.Combine(_fixtureDir, "with-table.docx");
        var outputPath = Path.Combine(_outputDir, "table-comment.docx");

        // Text that exists only in a table cell should not be found
        using (var session = _writer.Open(inputPath, outputPath))
        {
            Assert.Throws<ArgumentException>(() =>
                session.AddCommentByText("54.3 (12.1)", "AI", "Table-only text"));
        }
    }
