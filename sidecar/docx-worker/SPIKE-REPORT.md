...
Can Shoulders' AI agents read and annotate real HEOR Word manuscripts (.docx) — with citations, comments, tracked changes, tables — without breaking anything?
...
- Tested on 4 real HEOR manuscripts (Bayer protocols, Pfizer reports, 324KB–6.3MB)
...
### Not yet built
- Protocol between Shoulders and the writer (JSON over stdio)
- Integration into Panel tool system
- File duplication with naming convention
- Publishing as self-contained binary for Tauri sidecar
...
### Short-term (integration into Shoulders)
4. **Publish as self-contained .NET binary** — `dotnet publish -c Release -r osx-arm64 --self-contained` for Tauri sidecar.
...
├── testdata/                    fixtures + 4 real manuscripts
