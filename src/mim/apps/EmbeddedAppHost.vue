...
async function onMessage(event) {
  ...
  if (message.type === 'mim:ready') {
    frameReady.value = true
    return
  }
  if (message.type === 'mim:open-file' && message.path) {
    emit('openFile', String(message.path))
    return
  }
  if (message.type === 'mim:diagnostic' && message.message) {
    emit('diagnostic', String(message.message))
    return
  }
  if (message.type === 'mim:invoke') {
    await handleInvoke(message)
    return
  }
  if (message.type === 'mim:tool-response') {
...
async function handleInvoke(message) {
  ...
  try {
    ...
    postToFrame({ type: 'mim:result', id: message.id, result, error: null })
  } catch (cause) {
    postToFrame({
      type: 'mim:result',
      id: message.id,
