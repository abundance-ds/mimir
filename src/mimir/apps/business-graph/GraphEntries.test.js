...
describe('Graph entry table', () => {
  ...
  it('applies multiple kinds immediately, retains the popup, and restores focus on Escape', async () => {
    ...
    expect(control('graph-filter-kind').attributes('title')).toBe('Kind: Note, Project')
    expect(control('graph-filter-kind').text()).toBe('2')
    expect(wrapper.find('.graph-entries-tools').exists()).toBe(false)
    expect(control('graph-filter-kind').element.closest('th')).not.toBeNull()
    ...
    await control('graph-filter-clear-kind').trigger('click')
  ...
  it('finds projects by text, lists the current project first, and retains selected projects outside search', async () => {
    ...
    expect(control('graph-filter-project').attributes('title')).toBe('Project: Atlas, No project')
  ...
  it('keeps filter and sort controls available with zero results', async () => {
    ...
    expect(control('graph-filter-clear-kind').isVisible()).toBe(true)
    expect(wrapper.find('[data-graph-control="graph-empty-create"]').exists()).toBe(false)
    await control('graph-best-match').trigger('click')
    expect(control('graph-best-match').element.closest('th')).not.toBeNull()
