...
it('uses the requested scope and submits a changed location', async () => {
  ...
  await dialog.get('[aria-label="Save in"]').trigger('click')
...
it('selects and clears an optional company and retains it after a save error', async () => {
  const wrapper = openDialog()
  await wrapper.setProps({ companies: [{ id: 'acme', title: 'Acme' }] })
  const dialog = new DOMWrapper(document.querySelector('[data-scribe-create-dialog]'))
  await dialog.get('[aria-label="Company"]').trigger('click')
  await new DOMWrapper(document.querySelector('[data-graph-select-search]')).setValue('Acme')
  await new DOMWrapper(document.querySelector('[data-graph-select-option="acme"]')).trigger('click')
  await dialog.get('form').trigger('submit')
  expect(wrapper.emitted('submit').at(-1)[0].companyId).toBe('acme')
  await wrapper.setProps({ error: 'Could not save person' })
  expect(dialog.get('[aria-label="Company"]').text()).toContain('Acme')
  await dialog.get('[aria-label="Company"]').trigger('click')
  await new DOMWrapper(document.querySelector('[data-graph-select-option=""]')).trigger('click')
  await dialog.get('form').trigger('submit')
  expect(wrapper.emitted('submit').at(-1)[0]).not.toHaveProperty('companyId')
  await wrapper.setProps({ request: { kind: 'project', title: 'Atlas', scopeId: 'private' } })
  expect(dialog.find('[aria-label="Company"]').exists()).toBe(false)
  wrapper.unmount()
})
