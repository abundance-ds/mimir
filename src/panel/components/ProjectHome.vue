      ...
      <div class="ph-inner">
        <header class="ph-header">
          ...
          <div v-if="project.workspacePath" class="ph-path">
            ...
            <button class="ph-path-btn" title="Reveal in Finder" @click="reveal">
        ...
        <!-- Project Prompt (AGENTS.md — shared with team) -->
        <section class="ph-section">
          <label class="ph-label">Project Prompt</label>
          <template v-if="hasWorkspace">
            <p class="ph-hint">AGENTS.md in workspace root &middot; shared with team</p>
            <textarea
              v-model="systemPrompt"
              class="ph-textarea ph-textarea--mono"
              placeholder="Project-wide prompt for all team members..."
              rows="4"
              autocorrect="off"
              autocapitalize="off"
          ...
          <p v-else class="ph-hint">Link a folder to edit project prompts</p>
        </section>
        ...
        <section class="ph-section">
          ...
          <template v-if="hasWorkspace">
            <p class="ph-hint">Local only &middot; not in your repository</p>
            <textarea
          ...
          <p v-else class="ph-hint">Link a folder to edit personal instructions</p>
        </section>
...
import ProjectPermissions from './project/ProjectPermissions.vue'
import ProjectActivity from './project/ProjectActivity.vue'
...
async function saveSystemPrompt() {
  ...
  try {
    ...
    console.warn('[ProjectHome] Failed to save project prompt:', e)
  }
}
...
async function saveInstructions() {
  ...
  try {
    ...
    console.warn('[ProjectHome] Failed to save personal instructions:', e)
  }
}
...
async function reveal() {
  ...
  try {
    const { invoke } = await import('@tauri-apps/api/core')
    await invoke('reveal_in_finder', { path: project.value.workspacePath })
  } catch {}
}
