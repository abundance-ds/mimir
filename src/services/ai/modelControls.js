...
export function normalizeModelId(modelId, registry) {
  if (!modelId || modelId === 'auto') return null
  const legacyIds = registry?.legacyIds || {}
  return legacyIds[modelId] || modelId
}
...
export function resolveConcreteModel(registry, keyStatuses, modelId, feature = 'chat') {
  const normalized = normalizeModelId(modelId, registry)
