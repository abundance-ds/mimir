 ...
 * Ported from hugin-munin. Each entry is parsed independently —
 * one bad entry doesn't kill the batch.
 */
 ...
 * @param {string} bibtexText - raw BibTeX content
 * @returns {object[]} array of CSL-JSON entries
 */
export function parseBibtex(bibtexText) {
  const entries = splitEntries(bibtexText)
  const results = []
