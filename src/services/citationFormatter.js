 ...
 * Ported from hugin-munin. No external dependencies.
 *
 * Input:  CSL-JSON entry object
 * Output: formatted citation string
 */

export const STYLES = ['apa', 'chicago', 'mla', 'ieee', 'vancouver']
...
function mlaReference(csl) {
  const parts = []
  parts.push(mlaAuthors(getAuthors(csl)) + '.')
  parts.push(`"${csl.title}."`)
  if (csl['container-title']) {
    parts.push(italicize(csl['container-title']) + ',')
    const extras = []
    if (csl.volume) extras.push(`vol. ${csl.volume}`)
    if (csl.issue) extras.push(`no. ${csl.issue}`)
    if (extras.length) parts.push(extras.join(', ') + ',')
    parts.push(getYear(csl) + ',')
...
function mlaInline(csl) {
  const authors = getAuthors(csl)
  if (authors.length === 0) return `("${csl.title || '...'}")`
  if (authors.length === 1) {
    const page = csl.page ? ' ' + csl.page.split('-')[0] : ''
    return `(${authorLast(authors[0])}${page})`
  }
  if (authors.length === 2) {
    return `(${authorLast(authors[0])} and ${authorLast(authors[1])})`
  }
  return `(${authorLast(authors[0])} et al.)`
}
...
function mlaReferenceRich(csl) {
  const segs = []
  segs.push(seg(mlaAuthors(getAuthors(csl)) + '. '))
  segs.push(seg(`“${csl.title || ''}.” `))
  if (csl['container-title']) {
    segs.push(seg(csl['container-title'], true))
    segs.push(seg(', '))
...
function vancouverReferenceRich(csl, num) {
  ...
  segs.push(seg((csl.title || '') + '. '))
  if (csl['container-title']) {
    segs.push(seg(csl['container-title'] + '. ', false))
    segs.push(seg(`${getYear(csl)}`))
    const extras = []
    if (csl.volume) extras.push(csl.volume)
    if (csl.issue) extras.push(`(${csl.issue})`)
...
const styles = {
  ...
  mla: { reference: mlaReference, inline: mlaInline },
...
/**
 * Format a single reference as a bibliography entry string.
 * @param {object} entry - CSL-JSON entry
 * @param {string} style - one of STYLES
 * @param {number} [num] - sequence number (for IEEE/Vancouver)
 * @returns {string}
 */
export function formatCitation(entry, style = 'apa', num) {
  const formatter = styles[style] || styles.apa
  return formatter.reference(entry, num)
}

/**
 * Format a single reference as a bibliography entry.
 * Alias kept for compatibility with hugin-munin API.
 */
export function formatReference(entry, style = 'apa', num) {
  return formatCitation(entry, style, num)
}

/**
 * Format a single inline/parenthetical citation.
 */
export function formatInlineCitation(entry, style = 'apa', num) {
  const formatter = styles[style] || styles.apa
  return formatter.inline(entry, num)
}

/**
 * Format a single reference as rich segments: [{ text: string, italic: boolean }, ...]
 * Used for rendering with real italic marks instead of *asterisks*.
 */
export function formatReferenceRich(entry, style = 'apa', num) {
  const formatter = richStyles[style] || richStyles.apa
  return formatter(entry, num)
}

/**
 * Format a full bibliography from an array of CSL-JSON objects.
 */
export function formatBibliography(entries, style = 'apa') {
  const isNumbered = style === 'ieee' || style === 'vancouver'

  if (isNumbered) {
    return entries.map((csl, i) => formatCitation(csl, style, i + 1)).join('\n')
  }
  ...
  return sorted.map(csl => formatCitation(csl, style)).join('\n\n')
}
