<template>
  <div class="model-picker">
    ...
    <div v-if="isOpen" ref="floatingRef" class="picker-dropdown" :style="floatingStyles">
      <button
        v-for="(model, index) in flatModels"
        :key="model.id"
        class="picker-option"
        :class="{ selected: model.id === modelId, 'group-start': model._groupStart && index > 0 }"
...
const flatModels = computed(() => {
  ...
  return [...grouped, ...other].map((model) => {
    const groupStart = model.provider !== lastProvider
    lastProvider = model.provider
    return { ...model, _groupStart: groupStart }
  })
})
